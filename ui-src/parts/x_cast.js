...
const Cast={
  st:null, iv:0, full:false, fullIv:0, waitIv:0, qrUrl:'', sig:'', img:null,
  async poll(){
  ...
  openFull(){
    ...
    /* الصورة أصلاً شغالة قبل ما نفتح ملء الشاشة، فحدث onload ممكن ما ينطلق مرة ثانية.
       نتأكد من الصورة نفسها (naturalWidth) بدل ما نستنى الحدث. */
    const wait=el.querySelector('.cast-wait');
    const ready=()=>{ if(img.naturalWidth>0){ if(wait) wait.hidden=true; return true; } return false; };
    img.onload=ready;
    clearInterval(this.waitIv);
    if(!ready()) this.waitIv=setInterval(()=>{ if(!this.full||ready()) clearInterval(this.waitIv); },250);
  },
  closeFull(){
    this.full=false;
    clearInterval(this.waitIv); this.waitIv=0;
    const el=$('#castFull');
    const slot=$('#castSlot');
